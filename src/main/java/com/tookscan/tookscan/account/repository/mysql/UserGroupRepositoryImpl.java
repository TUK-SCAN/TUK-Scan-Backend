...
public class UserGroupRepositoryImpl implements UserGroupRepository {
    ...
    private final UserGroupJpaaRepository userGroupJpaaRepository;

    @Override
    public Set<Pair<UUID, Long>> findNotDuplicatedUserGroupInUserIdsAndGroupIds(List<UUID> userIds, List<Long> groupIds) {

        // 이미 등록된 UserGroup 조회
        Set<UserGroup> existingUserGroups = userGroupJpaaRepository.findAllByUserIdInAndGroupIdIn(userIds, groupIds);
    ...
    public void saveAll(List<UserGroup> userGroups) {
        userGroupJpaaRepository.saveAll(userGroups);
    }
}
